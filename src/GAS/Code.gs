...
function testDriveAccess() {
  ...
  var testFile;
  try {
    var testBlob = Utilities.newBlob('ProcureOS Drive Test', 'text/plain', 'ProcureOS_DriveTest_' + new Date().getTime() + '.txt');
...
function cleanupOldUploadSessions() {
  ...
  Object.keys(props).forEach(function(k) {
    if (k.startsWith('chu_') && !k.includes('_c')) {
      try {
        var meta = JSON.parse(props[k]);
        if (now - meta.created > twoHours) {
          for (var i = 0; i < meta.chunks; i++) {
            PropertiesService.getScriptProperties().deleteProperty(k + '_c' + String(i).padStart(4,'0'));
          }
